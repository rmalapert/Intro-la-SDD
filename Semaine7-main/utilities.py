...
def crop_image(img: Image.Image, crop_size: int = 170) -> Image.Image:
...
def crop_around_center(img, center=(64,64), crop_size=128):
    """Crop a PIL image to crop_size x crop_size."""
    ...
    right = round(center[1] + new_width / 2)
    bottom = round(center[0] + new_width / 2)
    # Then crop the image to that box
...
def df_cross_validate(df, sklearn_model, sklearn_metric, n=10, verbose=False):
    ...
    for train_index, test_index in SSS.split(X, Y):
        ...
        Xtrain = X[train_index]  # added line 
        Xtest = X[test_index]  # added line 
        Ytrain = Y[train_index]  # added line
        Ytest = Y[test_index]  # added line
        #2. Fit the model
        ...
        Ytest_predicted = sklearn_model.predict(Xtest)  # added line 
...
def analyze_model_experiments(result_df):
    # Identifier les classificateurs dont les performances de test sont inférieures à la médiane
    tebad = result_df["perf_te"] < result_df["perf_te"].median()
    # Identifier les classificateurs dont les performances d'entraînement sont inférieures à la médiane
    trbad = result_df["perf_tr"] < result_df["perf_tr"].median()
    # Identifier les classificateurs qui ont sur-appris
    overfitted = tebad & ~trbad
    # Identifier les classificateurs qui ont sous-appris
...
def my_foreground_filter(img):
    """
    applique foreground_redness_filter à img,
    inverse les valeurs booléennes si le fond est blanc avec la fonction invert_if_light_background
    ...
    Sortie:
    img_clean : l’objet de l’arrière-plan
...
def img_cropped(img):
    return img.crop(box=(35, 60, 170, 195))


def white_pixel(img: Image.Image, seuil=200) -> float:
    """Return the number of white pixel."""
    n = 0
    M = np.array(img)
    G = M[:, :, 1] * 1.0
    R = M[:, :, 0] * 1.0
    ...
    for i in range(len(R)):
        for j in range(len(G)):
            if (R[i][j] + B[i][j] + G[i][j])/3 > seuil:
                n += 1
    return n


def True_or_False(img):
    img_filter = foreground_redness_filter(img, theta=.85)
    M = np.array(img_filter)
    cpt = 0
    for i in range(len(M)):
        for j in range(len(M[i])):
            if M[i][j]:
                cpt += 1
    return 100*cpt/(len(M)*len(M[0]))
